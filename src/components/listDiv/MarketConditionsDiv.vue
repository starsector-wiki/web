...
const allMarketConditions = computed(() => marketConditionValues.map(it => typeof it === 'string' ? appData.getMarketConditionById(it) : it).filter(it => it !== undefined).sort(compareMarketCondition));
</script>
