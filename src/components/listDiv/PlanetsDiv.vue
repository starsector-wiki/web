...
const factionOptions = computed(() => {
  const basePlanets = filterType(
    filterSize(allPlanets.value, selectSize.value),
    selectType.value
  );
  const values = new Map<string, { id: string; name: string }>();
  for (const planet of allPlanets.value) {
    values.set(planet.factionId, {
      id: planet.factionId,
...
const sizeOptions = computed(() => {
  const basePlanets = filterType(
    filterFaction(allPlanets.value, selectFaction.value),
    selectType.value
  );
...
const typeOptions = computed(() => {
  const basePlanets = filterFaction(
    filterSize(allPlanets.value, selectSize.value),
    selectFaction.value
  );
  const values = new Map<string, { id: string; name: string }>();
    ...
    >
      <div class="card-item-content">
        ...
        <span :style="{ color: '#' + planet.faction.color }">
          {{ planet.name }}
          <span
            v-if="planet.market && !planet.market.planetConditionMarketOnly"
            >{{ planet.market.size }}</span
          >
        </span>
      </div>
    </q-btn>
